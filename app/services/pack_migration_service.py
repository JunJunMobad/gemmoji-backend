...
class PackMigrationService:
    ...
    async def migrate_pack_to_firestore(
        self, pack_data: PackMigrationData, user_id: str
    ) -> str:
        try:
            ...
            print(
                f"   📊 Fields: createdAt={created_at_ms}, downloadCount={pack_data.download_count}, emojiCount={pack_data.emoji_count}"
            )
            return doc_ref.id
    ...
    async def migrate_packs_from_json(
        self, packs_data: List[PackMigrationData], user_id: str
    ) -> List[str]:
        migrated_ids = []
        ...
        print(
            f"🎉 Migration completed! Successfully migrated {len(migrated_ids)}/{len(packs_data)} packs"
        )
        return migrated_ids
