...
class FirebaseService:
        ...
        try:
            ...
            if visibility:
                base_query = base_query.where(
                    filter=FieldFilter("visibility", "==", visibility)
                )
            ...
            if query:
                base_query = base_query.where(
                    filter=FieldFilter("prompt", ">=", query)
                ).where(filter=FieldFilter("prompt", "<=", query + "\uf8ff"))

            if sort == "popular":
                base_query = base_query.order_by(
                    "downloadCount", direction=firestore.Query.DESCENDING
                )
                base_query = base_query.order_by(
            ...
            for doc in results:
                ...
                if "createdAt" in doc_data and hasattr(
                    doc_data["createdAt"], "timestamp"
                ):
                    doc_data["createdAt"] = int(
                        doc_data["createdAt"].timestamp() * 1000
                    )
            ...
            next_cursor = (
                current_position + len(emojis) if len(emojis) == limit else None
            )
            has_more = len(emojis) == limit

            return {"emojis": emojis, "next_cursor": next_cursor, "has_more": has_more}
    ...
    async def list_packs(
        ...
        try:
            ...
            return {"packs": packs, "next_cursor": next_cursor, "has_more": has_more}
    ...
    async def increment_emoji_download_count(
        self, user_id: str, emoji_id: str
    ) -> Dict[str, Any]:
        try:
            doc_ref = (
                self.db.collection("emojis")
    ...
    async def update_emoji_visibility(
        self, user_id: str, emoji_id: str, visibility: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            doc_ref.update({"visibility": visibility})
