...
async def list_packs(
    query: Optional[str] = Query(None, description="Text to search in the pack name and description fields"),
    limit: int = Query(20, ge=1, le=100, description="Number of records per page"),
    cursor: Optional[int] = Query(None, description="Page offset - order number of first pack for next page"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to list packs: {error_msg}"
        )