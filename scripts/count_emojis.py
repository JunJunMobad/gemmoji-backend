...
async def count_emojis_collection_group(db):
    ...
    try:
        ...
        while True:
            ...
            print(f"Processed batch: {batch_count} emojis (Total so far: {total_count})")
...
def print_results(user_counts, total_emojis, category_stats):
    ...
    if category_stats:
        print(f"\nEMOJIS BY CATEGORY:")
        print("-" * 40)
        for category, count in sorted(category_stats['categories'].items(), key=lambda x: x[1], reverse=True):
            percentage = (count / category_stats['total'] * 100) if category_stats['total'] > 0 else 0
            print(f"   {category}: {count:,} emojis ({percentage:.1f}%)")
        
        print(f"\nEMOJIS BY VISIBILITY:")
        print("-" * 40)
        for visibility, count in sorted(category_stats['visibility'].items(), key=lambda x: x[1], reverse=True):
            percentage = (count / category_stats['total'] * 100) if category_stats['total'] > 0 else 0
        ...
        print(f"   Without predictionID: {category_stats['without_prediction_id']:,} emojis")
